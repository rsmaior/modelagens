...
class MasterGen():
    ...
    def buildSQL(self, dest, extension = None):
      ...
      sql.append(u"SET search_path TO pg_catalog,public,{0},{1};".format(master["schema_edgv"],master["schema_dominios"]))
      sql.append(u"")
      sql.append(u"CREATE TABLE public.db_metadata(")
      sql.append(u"\t edgvversion varchar(50) NOT NULL DEFAULT '{0}',".format(master["modelo"]))
      sql.append(u"\t dbimplversion varchar(50) NOT NULL DEFAULT '{0}',".format(master["versao"]))
      sql.append(u"\t CONSTRAINT edgvversioncheck CHECK (edgvversion = '{0}')".format(master["modelo"]))
      sql.append(u");")
      sql.append(u"INSERT INTO public.db_metadata (edgvversion, dbimplversion) VALUES ('{0}','{1}');".format(master["modelo"],master["versao"]))
      sql.append(u"")

      for dominio in master["dominios"]:
        sql.append(u"CREATE TABLE {0}.{1} (".format(master["schema_dominios"], dominio["nome"]))
        sql.append(u"\t code smallint NOT NULL,")
        sql.append(u"\t code_name text NOT NULL,")
        sql.append(u"\t filter text NOT NULL,") if "filtro" in dominio and dominio["filtro"] else None 
        sql.append(u"\t CONSTRAINT {0}_pk PRIMARY KEY (code)".format(dominio["nome"]))
        sql.append(u");")
        sql.append(u"")
      ...
      for classe in master["classes"]:
        for primitiva in classe["primitivas"]:
          sql.append(u"CREATE TABLE {0}.{3}_{1}_{2}(".format(master["schema_edgv"], classe["nome"], master["geom_suffix"][primitiva], classe["categoria"]))
          sql.append(u"\t {0} serial NOT NULL,".format(master["nome_id"]))

          for atributo in classe["atributos"]:
            if atributo["cardinalidade"] == "0..1":
              sql.append(u"\t {0} {1},".format(atributo["nome"], atributo["tipo"]))
            elif atributo["cardinalidade"] == "1..1":
              sql.append(u"\t {0} {1} NOT NULL,".format(atributo["nome"], atributo["tipo"]))
            elif atributo["cardinalidade"] == "0..*":
              sql.append(u"\t {0} {1}[],".format(atributo["nome"], atributo["tipo"]))
            elif atributo["cardinalidade"] == "1..*":
              sql.append(u"\t {0} {1}[] NOT NULL,".format(atributo["nome"], atributo["tipo"]))

          sql.append(u"\t {0} geometry({1}, {2}),".format(master["nome_geom"], primitiva, master["coord_sys"]))
          ...
          sql.append(u"\t WITH (FILLFACTOR = {0})".format(master["fill_factor"]))
          sql.append(u");")
          ...
          for atributo in classe["atributos"]:
            ...
            if "mapa_valor" in atributo:
              ...
              if atributo["cardinalidade"] == "0..1" or atributo["cardinalidade"] == "1..1":
                ...
                sql.append(u"\t REFERENCES {1}.{0} (code) MATCH FULL".format(atributo["mapa_valor"], master["schema_dominios"]))
                sql.append(u"\t ON UPDATE NO ACTION ON DELETE NO ACTION;")
                sql.append(u"")
    ...
    def buildRCO(self, dest, template):
        ...
        for classe in master["classes"]:
          classe["print_nome"] = "_".join([parte.title() for parte in classe["nome"].split("_")])
          ...
          for atributo in classe["atributos"]:
            ...
            if "valores" in atributo:
              ...
              for valor in atributo["valores"]:
                valor["print_nome"] = u"{0} ({1})".format(dominio_by_class_attribute[classe["nome"]][atributo["nome"]][valor["code"]]["value"],valor["code"])
                valor["descricao"] = dominio_by_class_attribute[classe["nome"]][atributo["nome"]][valor["code"]]["descricao"]
